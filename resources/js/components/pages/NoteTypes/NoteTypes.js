 ...
 const columns = [
  ...
  },
  {
  ...
  },
   {
      ...
      key: 'x',
      render: (data) =>(<> 
                           {verifyAccess('Note Types','Edit')?   
                          <Link to={API.defaults.frontURL+`/editNoteType?id=${data.id}`}>
                            <AiOutlineEdit style={{'marginRight':'10px'}}/>
                          </Link>:''  }
                           {verifyAccess('Note Types','Delete')?  
                          <Popconfirm title="Are you sure to delete?" onConfirm={()=>removeNoteType(data.id)}>
  ...
  const removeNoteType=(id)=>{
      const hide = message.loading('Deleting', 0);
      API.post('/removeNoteType',{'id':id})
                  .then(response=>{
                     setTimeout(hide, 0);
                    if(response.data.status){
                        setNoteTypes(response.data.noteTypes.map(row => ({
                                  key: row.id,
                                  id: row.id,
                                  name: row.name
                                })));
  ...
  useEffect(() => {
         ...
         if(!verifyAccess('Note Types','View')){
           navigate(API.defaults.frontURL+'/401');
         }
         setLoading(true);
        const hide = message.loading('Loading', 0);
            API.post('/getNoteTypes')
                  .then(response=>{
                     setTimeout(hide, 0);
                     setLoading(false);
                    if(response.data.status){
                ...
                <Body>
                  <h2>Note Types</h2>
                   <Divider orientation="right">
                    {verifyAccess('Note Types','Add')?
                    <Link to={API.defaults.frontURL+'/createNoteType'}>
                        <Button type="primary" shape="round" icon={<AiOutlinePlus style={{'marginTop':'-5%'}}/>} size={'Default'}> <span style={{'marginLeft':'5px'}}>Create Note Type</span>
                        </Button>
                    </Link>:''  }
                   </Divider>
                   {loading ?  <Skeleton />: <Table columns={columns} dataSource={noteTypes} onChange={onChange} />}
