  ...
  },
  {
  ...
  },
   {
      ...
      render: (data) =>(<> 
                          ...
                          <Popconfirm title="Are you sure to delete?" onConfirm={()=>removeDepartment(data.id)}>
      ...
      API.post('/removeDepartment',{'id':id})
                  .then(response=>{
                    ...
                    if(response.data.status){
                        setDepartments(response.data.departments.map(row => ({
                                  key: row.id,
                                  id: row.id,
                                  name: row.name
                                })));
              ...
              <>
                <Body>
                  <h2>Departments</h2>
                   ...
                   {loading ?  <Skeleton />: <Table columns={columns} dataSource={departments} onChange={onChange} />}
