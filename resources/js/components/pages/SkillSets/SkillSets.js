...
export const SkillSets = (props) => {
 ...
 const columns = [
  ...
  },
  {
  ...
  },
   {
      ...
      render: (data) =>(<>
                          ...
                          <Popconfirm title="Are you sure to delete?" onConfirm={()=>removeSkillSet(data.id)}>
  ...
  const removeSkillSet=(id)=>{
      const hide = message.loading('Deleting', 0);
      API.post('/removeSkillSet',{'id':id})
                  .then(response=>{
                     setTimeout(hide, 0);
                    if(response.data.status){
                        setSkillSets(response.data.skillSets.map(row => ({
                                  key: row.id,
                                  id: row.id,
                                  name: row.name
                                })));
              ...
              <>
                <Body>
                  <h2>Skill Sets</h2>
                   ...
                   {loading ?  <Skeleton />: <Table columns={columns} dataSource={skillSets} onChange={onChange} />}
...
export default connect(mapStateToProps, {})(SkillSets)