...
export const CreateCandidate = (props) => {
    ...
    const onEmailBlurHandler = (e) => {
        if(e.target.value){
            ...
            setCheckingMail(true);
             API.post('/checkDuplicacyWithEmail',{'email':e.target.value})
                  .then(response=>{
     ...
     const onMobileBlurHandler = (e) => {
          if(e.target.value){
                ...
                setCheckingPhone(true);
                 API.post('/checkDuplicacyWithMobileNo',{'mobileNo':e.target.value})
                      .then(response=>{
    ...
    useEffect(() => {
        ...
        API.post('/getMetadataForCandidateForm',{'user_id':userDetails.id})
              .then(response=>{
                 formDisabled=false; 
                setTimeout(hide, 0);
                        ...
                        <Card>
                            ...
                            <Form form={form} disabled={formDisabled} name="job" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} initialValues={{ rating: 3}} onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete="off">
                                ...
                                <Row>
                                    ...
                                    <Col span={12}>
                                        <Form.Item label="Email" name="email" rules={[{type: 'email',message: 'The input is not valid E-mail!'},{ required: true, message: 'Email is required!' }]}  hasFeedback={hasEmailFeedback} validateStatus={checking_email ?"validating"  : email_exists ? "error" : "success"} help={checking_email ?  "Checking email"  : email_exists ?  "email already exists" : ""}  >
                                            <Input type="email"  onBlur={onEmailBlurHandler} />
                                        </Form.Item>
                                    </Col>
                                    <Col span={12}>
                                        <Form.Item label="Mobile" name="mobileNo" rules={[{ required: true, message: 'Mobile is required!' }]}  style={{'width':'100%'}} hasFeedback={hasPhoneFeedback} validateStatus={checking_phone ?"validating"  : phone_exists ? "error" : "success"} help={checking_phone ?  "Checking Phone"  : phone_exists ?  "phone number already exists" : ""} >
