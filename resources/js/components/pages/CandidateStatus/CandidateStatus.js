...
export const CandidateStatus = (props) => {
 ...
 const columns = [
  ...
  },
  {
  ...
  },
   {
      ...
      render: (data) =>(<>  
                          ...
                          <Popconfirm title="Are you sure to delete?" onConfirm={()=>removeCandidateStatus(data.id)}>
  ...
  const removeCandidateStatus=(id)=>{
      const hide = message.loading('Deleting', 0);
      API.post('/removeCandidateStatus',{'id':id})
                  .then(response=>{
                     setTimeout(hide, 0);
                    if(response.data.status){
                        setStatusList(response.data.candidate_status_list.map(row => ({
                                  key: row.id,
                                  id: row.id,
                                  name: row.name
                                })));
  ...
  useEffect(() => {
        ...
        const hide = message.loading('Loading', 0);
            API.post('/getCandidateStatusList')
                  .then(response=>{
                     setTimeout(hide, 0);
                     setLoading(false);
                    if(response.data.status){
...
export default connect(mapStateToProps, {})(CandidateStatus)