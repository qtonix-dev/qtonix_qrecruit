...
export const EditCandidateStatus = (props) => {
     ...
     const verifyAccess=(section, action)=>{
      ...
      useEffect(() => {
         ...
         if(!verifyAccess('Candidate Status','Edit')){
           navigate(API.defaults.frontURL+'/401');
         }
        setLoading(true);
         console.log(loading);
          const hide = message.loading('Loading', 0);
            API.post('/getCandidateStatusDetails',{'id':params.get("id")})
                  .then(response=>{
                     setTimeout(hide, 0);
       ...
       const onFinish = (formData) => {
           const hide = message.loading('Loading', 0);
              API.post('/updateCandidateStatus',formData)
                  .then(response=>{
                     setTimeout(hide, 0);
                    if(response.data.status){
                         navigate(API.defaults.frontURL+'/candidateStatus');
                      }else{
                          message.error('Error occurred');
                      }
...
export default connect(mapStateToProps, {})(EditCandidateStatus)