...
export const JobTypes = (props) => {
 ...
 const columns = [
  ...
  },
  {
  ...
  },
   {
      ...
      render: (data) =>(<> 
                          ...
                          <Popconfirm title="Are you sure to delete?" onConfirm={()=>removeJobType(data.id)}>
  ...
  const removeJobType=(id)=>{
      ...
      API.post('/removeJobType',{'id':id})
                  .then(response=>{
                     setTimeout(hide, 0);
                    if(response.data.status){
                        setJobTypes(response.data.jobTypes.map(row => ({
                                  key: row.id,
                                  id: row.id,
                                  name: row.name
                                })));
              ...
              <>
                <Body>
                  <h2>Job Types</h2>
                   ...
                   {loading ?  <Skeleton />: <Table columns={columns} dataSource={jobTypes} onChange={onChange} />}
