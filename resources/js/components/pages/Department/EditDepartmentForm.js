...
export const EditDepartmentForm = (props) => {
     ...
     const [department_details, setDepartmentDetails] = useState({});
     ...
     const verifyAccess=(section, action)=>{
      ...
      useEffect(() => {
         ...
         if(!verifyAccess('Departments','Edit')){
           navigate(API.defaults.frontURL+'/401');
         }
        setLoading(true);
         console.log(loading);
          const hide = message.loading('Loading', 0);
            API.post('/getDepartmentDetails',{'id':params.get("id")})
                  .then(response=>{
                     setTimeout(hide, 0);

                     
                    if(response.data.status){
                        setDepartmentDetails(response.data.department_details);
                         setLoading(false);
                      }else{
       ...
       const onFinish = (formData) => {
           const hide = message.loading('Loading', 0);
              API.post('/updateDepartmentDetails',formData)
                  .then(response=>{
                     setTimeout(hide, 0);
                    if(response.data.status){
                         navigate(API.defaults.frontURL+'/departments');
                      }else{
                          message.error('Error occurred');
                      }
                  ...
                  <Row>
                      <Col span={20} offset={2}>
                        <Card>
                           ...
                           {loading ?  
                             <Skeleton /> :
                             <Form name="department" labelCol={{span: 8}}  initialValues={{'id': department_details.id,'name': department_details.name}} wrapperCol={{span: 16}}  onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete="off">
                               <Form.Item label="" name="id" >
                                        <Input type="hidden"/>
                               </Form.Item>    
                               <Row>
                                  <Col span={12}>
                                      <Form.Item label="Deparment Name" name="name"  rules={[ { required: true, message: 'Name is required!' }]}  >
                                        <Input  />
                                      </Form.Item>
                                  </Col>
                               </Row>
