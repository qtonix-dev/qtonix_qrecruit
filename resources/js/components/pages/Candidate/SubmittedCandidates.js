...
export const SubmittedCandidates = (props) => {
     ...
     const verifyAccess=(section, action)=>{
       return Object.keys(userDetails.access).indexOf(section)>=0 && Object.values(userDetails.access)[Object.keys(userDetails.access).indexOf(section)].includes(action);
        ...
        useEffect(() => {
         ...
         if(!verifyAccess('Submitted Candidates','View')){
           navigate(API.defaults.frontURL+'/401');
         }
        const hide = message.loading('Loading', 0);
        setLoading(true);
            API.post('/getSubmittedCandidateLists',{user_id:userDetails.id})
                  .then(response=>{
                    setLoading(false);
                     setTimeout(hide, 0);
                    if(response.data.status){
...
export default connect(mapStateToProps, {})(SubmittedCandidates)